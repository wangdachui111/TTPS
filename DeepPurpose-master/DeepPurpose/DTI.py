...
def virtual_screening_1(X_repurpose, target, model, drug_names=None, target_names=None,
					  result_folder="./result/", convert_y=False, output_num_max=10, verbose=True):
	# X_repurpose: a list of SMILES string
	# target: a list of targets
	if isinstance(target, str):
...
class DBTA:
	...
	def test_1(self, data_generator, model, repurposing_mode=False, test=True):
		y_pred = []
		y_label = []
		model.eval()
	...
	def predict_1(self, df_data):
		...
		score = self.test_1(generator, self.model, repurposing_mode=True)
		return score
