...
plot_fun <- function(res_all,Module,res_name,p_name){
  if (length(res_name) >1) {
    hist_plot <- plot_hist(res_all,Module,res_name,p_name)
    corr_plot <- plot_corr(res_all,Module,res_name,p_name)
    fq_plot <- plot_fq(res_all,Module,res_name,p_name)
    #venn_plot <- plot_venn(res_all,Module)
    heat_plot <- plot_heatmap(res_all,Module,res_name,p_name)
...
#correlation
plot_corr <- function(res_all,Module,res_name,p_name){
  res_all_dat <- res_all[[Module]]
  res_ncs <- apply(data.frame(res_all_dat[,endsWith(colnames(res_all_dat),"NCS")]),2,as.numeric)
  ...
  for (i in 1:ncol(num_p)) {
    ...
    assign(lis_p[i],ggplot(res_ncs, aes_string(x = aes_xy[1],y= aes_xy[2]))+
             ...
             theme(axis.text = element_text(size = 18),legend.title = element_text(size = 25),title = element_text(size = 25),
                   panel.grid.major=element_blank(),panel.grid.minor=element_blank(),text=element_text(family="serif"))+
             annotate("text",x=lab_num1,y=lab_num2,label = paste("R =",cor_value,sep = " "),color="black",size=8,family = "serif")
  ...
  plot_list
  ggsave(paste0("../plot_result/",p_name,"_corr.pdf"),plot_list)
  return(plot_list)
}
...
plot_fq <- function(res_all,Module,res_name,p_name) {
  ...
  for (i in 1:ns) {
    ...
    assign(lis_p[i],
           ggplot(res_ncs,aes_string(x= aes_x))+geom_histogram(aes(y=..density..),colour="white",fill="lightblue")+
             theme_bw()+theme(title = element_text(size = 17),axis.text = element_text(size = 18),
                              axis.title = element_text(size = 25),text=element_text(family="serif"))+
             labs(x= paste(res_name[i],"_NCS"))  
    )
  }
  plot_list <- eval(parse(text = paste0("cowplot::plot_grid(",noquote(paste0(noquote(lis_p),",",collapse = "")),"nrow = 1)")))
  plot_list
  ggsave(paste0("../plot_result/",p_name,"_fq.pdf"),plot_list)
  return(plot_list)
}
...
plot_heatmap <- function(res_all,Module,res_name,p_name){
  res_all <- res_all[[Module]]
  res_ncs <- apply(data.frame(res_all[,endsWith(colnames(res_all),"NCS")]),2,as.numeric)
  ...
  p <- ggplot(res_ncs,aes(x = group,y= genename,fill = value))+
    ...
    theme(axis.text.y = element_blank(),axis.title.y = element_blank(),axis.ticks = element_blank(),
          axis.title.x.bottom = element_blank(),axis.title = element_text(size = 25),
          axis.line = element_blank(),axis.text = element_text(size = 25,face = "bold"),legend.text = element_text(size = 18),
          axis.title.x = element_text(size = 30),legend.title = element_text(size = 22),panel.grid = element_blank(),
          panel.border = element_blank(),text=element_text(family="serif"))
  p
  ggsave(paste0("../plot_result/",p_name,"_heatmap.pdf"),p)
  return(p)
}
...
plot_hist <- function(res_all,Module,res_name,p_name){
  res_all <- res_all[[Module]]
  ...
  p1 <- ggplot(dat_hist,aes(x = group,y=count,fill = factor))+
    ...
    theme_bw()+
    theme(axis.text = element_text(size = 25,face = "bold"),panel.grid = element_blank(),
          legend.text = element_text(size = 25),legend.title = element_text(size = 25),
          strip.background = element_rect(fill = NULL),
          title = element_text(size = 25),text=element_text(family="serif"))
  p1
  ggsave(paste0("../plot_result/",p_name,"_hist.pdf"),p1)
  return(p1)
}
