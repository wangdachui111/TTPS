...
def calculate_small_network_Fvalue1(params):
	node_set_name, node_set, p, n, bg, verbose = params[0], params[1], params[2], params[3], params[4], params[5]
	runtime = time.time()
	intersect = [nodes for nodes in node_set if nodes in kernel.index]
	AUPRCs = []
	...
	for i in range(n):																				# Number of times to run the sampling
		...
		for node in intersect_non_sample_sorted:														# Slide down sorted nodes by summed prop value by nodes that are in intersect_non_sample
			...
			precision.append(TP/float(y_actual.ix[:node].shape[0]))										 	# Calculate precision ( TP / TP+FP ) and add point to curve
			recall.append(TP/float(TP+FN))																  	# Calculate recall ( TP / TP+FN ) and add point to curve
			p1 = TP/float(y_actual.ix[:node].shape[0])
			r1 = TP/float(TP+FN)
			if TP == 1.0:
				Fmax_index = node
				Fmax = (2*p1*r1/(p1+r1))
		...
		AUPRCs.append(metrics.auc(recall, precision))												   		# Calculate Area Under Precision-Recall Curve (AUPRC)
		Fscore.append(Fmax)
		Fnode.append(y_actual.ix[:Fmax_index].index)
		pr_value.append(AUPRCs)
	if verbose:
		print 'AUPRC Analysis for given node set', '('+repr(len(intersect))+' nodes in network) complete:', round(time.time()-runtime, 2), 'seconds.'
	return [node_set_name,pr_value]

def calculate_small_network_Fvalue(params):
	node_set_name, node_set, p, n, bg, verbose = params[0], params[1], params[2], params[3], params[4], params[5]
	runtime = time.time()
	intersect = [nodes for nodes in node_set if nodes in kernel.index]
	AUPRCs = []
	...
	for i in range(n):																				# Number of times to run the sampling
		...
		Fscore.append(Fmax)
		Fnode.append(y_actual.ix[:Fmax_index].index)
		pr_value.append(pr_max)
	if verbose:
		print 'AUPRC Analysis for given node set', '('+repr(len(intersect))+' nodes in network) complete:', round(time.time()-runtime, 2), 'seconds.'
	return [node_set_name,pr_value]

def small_network_Fvalue_wrapper(net_kernel, genesets, genesets_p, n=30, cores=1, bg=None, verbose=True):
	# Construct params list
	if bg is None:
		bg_intersect = list(net_kernel.index)
	else:
	...
	if cores == 1:
		...
		AUPRC_results = []
		for params_list in AUPRC_Analysis_params:
			AUPRC_results.append(calculate_small_network_Fvalue(params_list))
	else:
		# Initialize worker pool
		pool = Pool(cores, global_var_initializer, [net_kernel])
		# Run the AUPRC analysis for each geneset
...
def small_network_Fvalue_wrapper1(net_kernel, genesets, genesets_p, n=30, cores=1, bg=None, verbose=True):
	# Construct params list
	if bg is None:
		bg_intersect = list(net_kernel.index)
	else:
	...
	if cores == 1:
		...
		pool.close()
	# Construct AUPRC results
	geneset_AUPRCs = pd.DataFrame({result[0]: result[1] for result in AUPRC_results})
	return geneset_AUPRCs




def calculate_small_network_Fscore(params):
	node_set_name, node_set, p, n, bg, verbose = params[0], params[1], params[2], params[3], params[4], params[5]
	runtime = time.time()
	intersect = [nodes for nodes in node_set if nodes in kernel.index]
	intersect_non_bg = [node for node in bg if node not in intersect]
	bg_intersect_mean = kernel.ix[intersect][intersect_non_bg].mean().sort_values(ascending=False)
	AUPRCs = []
	Fscore = []
	Fnode = []
	pr_value = []
	re_value = []
	#pr_value = pd.DataFrame(index=range(30),columns=node_set_name)
	sample_size = int(round(p*len(intersect)))
	random.seed(123)
	for i in range(n):																				# Number of times to run the sampling
		sample = random.sample(intersect, sample_size)													# get node set sample
		intersect_non_sample = [node for node in intersect if node not in sample]					   	# nodes in intersect not in sample
		bg_non_sample = [node for node in bg if node not in sample]							 			# nodes in background gene list not in sample
		bg_sample_mean = kernel.ix[sample][bg_non_sample].mean().sort_values(ascending=False)				# summed prop value for all nodes in background
		y_actual = pd.Series(0, index=bg_sample_mean.index, dtype=int)									# nodes sorted by mean prop value
		y_actual.ix[intersect_non_sample]+=1															# which nodes in sorted list are in intersect_non_sample
		intersect_non_sample_sorted = y_actual[y_actual==1].index									   	# intersect_non_sample sorted
		TP, FN = 0, len(intersect_non_sample_sorted)													# initialize precision and recall curves
		precision, recall = [1], [0]																	# initialize true positives and false negatives
		for node in intersect_non_sample_sorted:														# Slide down sorted nodes by summed prop value by nodes that are in intersect_non_sample
			...
			p1 = TP/float(y_actual.ix[:node].shape[0])
			r1 = TP/float(TP+FN)
			Fscore.append((1.09*p1*r1/(0.09*p1+r1)))
			if TP == 1.0:
				Fmax_index = node
				Fmax = (1.09*p1*r1/(0.09*p1+r1))
				p2 = p1
				r2 = r1
			else:
				if Fmax >= (1.09*p1*r1/(0.09*p1+r1)):
					continue
				else:
					Fmax_index = node
					Fmax_mean_value = bg_sample_mean[Fmax_index]
					Fmax = ((1.09*p1*r1/(0.09*p1+r1)))
					p2 = p1
					r2 = r1
		AUPRCs.append(metrics.auc(recall, precision))												   		# Calculate Area Under Precision-Recall Curve (AUPRC)
		...
		Fscore.append(Fmax)
		if i == 0:
			Fmax_node = y_actual.ix[:Fmax_index].index
			#Fnode_len = len(Fmax_node)
		else:
			Fmax_temp = y_actual.ix[:Fmax_index].index
			Fmax_node = [node for node in Fmax_temp if node in Fmax_node]
			#Fnode_len = len(Fmax_node)
		Fnode.append(Fmax_mean_value)
	Fnode_site = np.mean(Fnode)
	#Fnode_site_value = list(bg_intersect_mean[bg_intersect_mean >= Fnode_site])
	Fnode_predict_node = list(bg_intersect_mean[bg_intersect_mean >= Fnode_site].index)
	Fnode_predict = []
	Fnode_predict.append(node_set_name)
	Fnode_predict.append(Fnode_predict_node)
	if verbose:
		print 'AUPRC Analysis for given node set', '('+repr(len(intersect))+' nodes in network) complete:', round(time.time()-runtime, 2), 'seconds.'
	return Fnode_predict


def small_network_Fscore_wrapper(net_kernel, genesets, genesets_p, n=30, cores=1, bg=None, verbose=True):
	# Construct params list
	if bg is None:
		bg_intersect = list(net_kernel.index)
	else:
	...
	if cores == 1:
		...
		global_var_initializer(net_kernel)
		# Calculate AUPRC values for all gene sets
		AUPRC_results = pd.DataFrame()
		for params_list in AUPRC_Analysis_params:
			Fnode_result = calculate_small_network_Fscore(params_list)
			AUPRC_results = pd.concat([AUPRC_results, pd.DataFrame(Fnode_result[1], columns=[Fnode_result[0]])], ignore_index=True, axis=1)
	else:
		# Initialize worker pool
		pool = Pool(cores, global_var_initializer, [net_kernel])
		# Run the AUPRC analysis for each geneset
		...
		pool.close()
	# Construct AUPRC results
	#geneset_AUPRCs = pd.DataFrame()
	#geneset_AUPRCs = geneset_AUPRCs.append(({result[0],result[1] for result in AUPRC_results}))
